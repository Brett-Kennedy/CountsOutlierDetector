...
NUM_DATASETS_EVALUATED = 100  #todo: seems to stop at 124 when set to 200
TEST_SINGLE_DATASET = ""
...
def run_tests(datasets):
    ...
    for dataset_dict in datasets:
        ...
        x = dataset_dict['X']
        det = CountsOutlierDetector(max_num_combinations=MAX_NUM_COMBINATIONS, results_folder=results_folder, results_name=dataset_name, run_parallel=RUN_PARALLEL)
...
def output_run_summary(run_summary_df):
    ...
    print("avg Percent Flagged as 6d: ", run_summary_df['Percent Flagged as 6d'].astype(float).mean())
    ...
    heights_arr = [run_summary_df['Percent Flagged as 1d'].astype(float).mean(),
                   ...
                   run_summary_df['Percent Flagged as 6d'].astype(float).mean()]
    fig, ax = plt.subplots()
    ax.bar(
        x=range(1, 7), # todo: in this and other plots, have a variable for the max dimensions examined.
        height=heights_arr,
        tick_label=[f"avg % Flagged examining {x}d: " for x in range(1, 7)]
    )
    ...
    heights_arr = [run_summary_df['Percent Flagged up to 1d'].astype(float).mean(),
                   ...
                   run_summary_df['Percent Flagged up to 6d'].astype(float).mean()]
    fig, ax = plt.subplots()
    ax.bar(
        x=range(1, 7),
        height=heights_arr,
        tick_label=[f"avg % Flagged examining up to {x}d: " for x in range(1, 7)]
    )
    plt.xticks(rotation=45, ha='right')
    ax.set_yticks(range(0, 110, 10))
    ...
    heights_arr = [100.0,
                   100.0,
                   run_summary_df['Checked_3d'].sum() * 100.0 / len(run_summary_df),
                   run_summary_df['Checked_4d'].sum() * 100.0 / len(run_summary_df),
                   run_summary_df['Checked_5d'].sum() * 100.0 / len(run_summary_df),
                   run_summary_df['Checked_6d'].sum() * 100.0 / len(run_summary_df)
                   ]
    fig, ax = plt.subplots()
    ax.bar(
        x=range(1, 7),
        height=heights_arr,
        tick_label=[f"% Datasets examining up to {x}d: " for x in range(1, 7)]
    )
    plt.xticks(rotation=45, ha='right')
    ax.set_yticks(range(0, 110, 10))
...
def sythetic_test_1():
    """
    This is a simple demonstration of the CountsOutlierDetector API. It creates a dataset with 5000 rows, 10 numeric
    columns with uniform distributions. It explicitly sets one outlier in Row 0 and 2 (unrelated) outliers in Row 1.
    It then calls CountsOutlierDetector's predict() API then other APIs to explain the results.
    """

    det = CountsOutlierDetector(results_folder='Results', results_name='toy', verbose=True)
    ...
    print()
    print("Breakdown All Rows:")
    print(results['Breakdown All Rows'])

    print()
    print("Breakdown Flagged Rows:")
    print(results['Breakdown Flagged Rows'])

    print()
    print("Flagged Summary:")
    print(results['Flagged Summary'])

    print()
    print("Most Flagged Rows:")
    most_flagged_df = det.get_most_flagged_rows()
    print(most_flagged_df)
...
def synthetic_test_categorical():
    a_vals = ['a1', 'a2']
    b_vals = ['b1', 'b2', 'b3']
    c_vals = ['c1', 'c2', 'c3', 'c4']
    ...
    nrows = 10_000
    df = pd.DataFrame({
        'a': np.random.choice(a_vals, nrows, p=[0.9, 0.1]),
        'b': np.random.choice(b_vals, nrows, p=[0.9, 0.09, 0.01]),
        'c': np.random.choice(c_vals, nrows, p=[0.1, 0.2, 0.3, 0.4]),
        'd': np.random.choice(d_vals, nrows, p=[0.1, 0.1, 0.1, 0.2, 0.5]),
    })
...
def real_test_openml():
    data = fetch_openml('abalone', version=1)
    df = pd.DataFrame(data.data, columns=data.feature_names)

    det = CountsOutlierDetector(n_bins=3, max_num_combinations=100_000, verbose=True)
    results = det.predict(df)
    print(results['Flagged Summary'])

    most_flagged_df = det.get_most_flagged_rows()
    if most_flagged_df is not None:
        print(most_flagged_df.head())

    det.explain_features(['Diameter', 'Shucked_weight', 'Shell_weight'])
    # det.explain_row(3409)


if __name__ == "__main__":
    # sythetic_test_1()
    # sythetic_test_2()
    # synthetic_test_categorical()
    # real_test_1()
    real_test_openml()

...
Binning numeric data does introduce a small number of decisions, but also eliminates the distance metrics required by numeric-based outlier detectors, and the need to numerically encode categorical columns. Note that with numeric outlier detectors, it is necessary to encode the categorical features, which creates a similar set of decisions, which also affect the performance of the detectors. 

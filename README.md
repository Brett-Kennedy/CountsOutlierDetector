...
Despite the common business need to understand and assess outliers within tabular datasets, most outlier detectors are very much blackboxes, often as much as many classifiers and regressors. While there has been a great deal of work developing interpretable predictors and providing explanations for blackbox predictors such as neural networks, there has not been as significant an effort with outlier detection. 
...
If the check_marginal_probs parameter is set to True, then combinations of values will only be flagged if they are both rare, and rare considering the marginal probalities. Doing this can introduce some complications as it allows, for example, cases where the count is only very slightly below what would be expected based on the marginal probabilities. Further tuning is possible, but not supported in the present version. 

...
The algorithm provides clear outliers as it emphasizes outliers in low-dimensionalities. In some sense, this may be considered a limitation, as it tends to flag outliers in low dimensionalities and not in high dimensionalities. However, this may be considered more a benefit, as we believe the vast majority of outliers may be described as outliers using few features, which is what this tool seeks to do. The algorithm allows the tool to describe each outlier in as few dimensions as possible, which then presents each outlier as clearly as possible. 

...
When considering two or more dimensions, there are at least two ways an instance may be considered rare: 1) Where the number of instances with a given pair or values is very low; 2) where the number of instances with a given pair of values is low considering the marginal probabilities of the relevant features. For example, assume a row has value A<sub>15</sub> in Column A and B<sub>4</sub> in Column B, and there are only 200 rows with this pair of values, representing 1% of the dataset. If we consider the marginal probabilities then we also look at the frequency of A<sub>15</sub> and B<sub>4</sub>. If A<sub>15</sub> covers 60% of the rows, and B<sub>4</sub> covers 45% of the rows, then we would expect rows with both values to cover 0.6 * 0.45 = 27% of the rows. Anything less could be considered unusual. So, rows with A<sub>15</sub> in Column A and B<sub>4</sub> in Column B could be flagged simply because this combination is less common than expected, similar to egg-laying animals with fur. Conversely, we can have combinations that are very rare, but may not be flagged, if considering marginal probabilities, if the expected frequency, based on the marginal probabilites of A<sub>15</sub> and B<sub>4</sub> would estimate an even lower count.

By default, COD, does not consider marginal probabilities, though this may be enabled if desired. Considering marginal probabilities simply tests for another type of outlier, also quite valid. 

Considering marginal probabilities can make the plots more difficult, as some combinations may not be flagged due to the marginal probabilties of the features invovled. As well, it is, arguably, more intuitive to simply examine the counts of each combination, regardless of the expected counts given the marginal probabilities of the relevant features. 

And, get into some just below or above anyway, which can lead to more tuning. 

Explain: by default does not consider marginal probabilities. If does, get only 1 type of outlier, not both (for 2d+). Considering marginal probs can make the plots difficult to follow, as some values look rare, but are not flagged. Here we flag, by default, any cell that’s rare, regardless. Is clear, and doesn’t miss the other type of outlier, which is probably as relevant, though may not want. Can flag if values in each feature a little odd. If say, 4d, then 4 all moderately unusual, though don’t quite qualify in any smaller dimensionality. Or if values all fairly common, but the combination is unusual. 
...
Conclude – CountsOutlierDetector is competitive with IF, at least as measured with respect to doping (a very reasonable metric, but not the only) on this set of datasets , and interpretable, similar times, though can let run longer. So just use this! No, don’t say that. 
...
Confirms, checking small number of dimensions is usually quite sufficient. This allows tools like COD to check a small number of dimensions, examining these dimensions exhaustively and without randomness. 

...
class CountsOutlierDetector:
    ...
    def __init__(self,
                 ...
                 run_parallel=False):
        """
        :param n_bins: The number of bins used to reduce numeric columns to a small set of ordinal values.
        :param max_dimensions: The maximum number of columns examined at any time. If set to, for example, 4, then
            the detector will check for 1d, 2d, 3d, and 4d outliers, but not outliers in higher dimensions.
        :param max_num_combinations: This, as well as max_dimensions, determines the maximum number of dimensions that
            may be examined at a time. When determining if the detector considers, for example, 3d outliers, it
            examines the number of columns and average number of unique values per column and estimates the total
            number of combinations. If this exceeds max_num_combinations, the detector will not consider spaces of this
            dimensionality or higher.
        :param min_values_per_column: The detector excludes from examination any columns with less than this number of
            unique values
        :param max_values_per_column: The detector excludes from examination any columns with more than this number of
            unique values
        :param results_folder: If specified, the output will be written to a .csv file in this folder. If unspecified,
            no output file will be written. Required if results_name is specified.
        :param results_name: Optional text to be included in the names of the output files, if created. The output file
            names will also include the date and time, to allow multiple to be created without over-writing previous
            output files.
        :param run_parallel: If set True, the process will execute in parallel, typically allowing some performance
            gain.
        """
        ...
        self.data_df = None
        self.dimensions_examined = None  # This may be less than max_dimensions given max_num_combinations

        # Set the seed to ensure consistent results from run to run on the same dataset.
        np.random.seed(0)
        random.seed(0)

    def get_col_types_arr(self):
        """
    ...
    def get_col_value(self, col_idx, value_idx):
        ...
        range of values in each bin, and so the bin id is returned to approximate the orginal value.
        """
    ...
    def predict(self, input_data):
        ...
        def get_1d_stats():
            ...
            for i in range(num_cols):
                ...
                col_threshold = (1.0 / num_unique_vals[i]) * self.divisor
                ...
                for v in unique_vals[i]:  # loop through each unique value in the current column.
                    frac = data_df.iloc[:, i].tolist().count(v) / num_rows
                    ...
                    if rare_values_flag:
                        ...
                        for r in rows_matching[0]:
                            outliers_1d_arr[r] += 1
                            outliers_explanation_arr[r] += f'[Column: {data_df.columns[i]}, ' + \
                        ...
                        for j_vals_idx in range(len(fractions_2d[i][j][i_vals_idx])):
                            ...
                            if rare_value_flag:
                                ...
                                for r in row_nums:
                                    ...
                                    outliers_explanation_arr[r] += f" [[Columns: {data_df.columns[i]} and " +\
                                        f"{data_df.columns[j]} Values: {self.get_col_value(i, i_vals_idx)} and " +\
        ...
        def get_6d_stats(num_combinations):
            ...
            update_output_msg("6d: Outlier Counts by score", outliers_5d_arr)
        ...
        self.col_types_arr = self.get_col_types_arr()
        numeric_col_names = []
        for c in range(len(self.col_types_arr)):
            if self.col_types_arr[c] == 'N':
                numeric_col_names.append(self.data_df.columns[c])
        ...
        if len(numeric_col_names):
            ...
            for num_idx, col_name in enumerate(numeric_col_names):
                self.data_df[col_name] = Xt[:, num_idx].astype(int)

        # Remove any columns with 1 unique value or a very large number of unique values
        drop_col_names_arr = []
        for c in range(len(input_data.columns)):
        ...
        if num_combinations > self.max_num_combinations:
            output_msg += (f"\n\nCannot determine 3d outliers given the number of columns ({num_cols}) and" +
                  f"number of unique values in each. Estimated number of combinations: {round(num_combinations):,}")
            outliers_3d_arr = [0] * num_rows
        ...
        if num_cols < 4:
            ...
            output_msg += f"\n\nCannot determine 4d outliers given the number of columns ({num_cols}) and number of unique values in each. Estimated number of combinations: {round(num_combinations):,}"
        else:
        ...
        if num_cols < 5:
            ...
            output_msg += f"\n\nCannot determine 5d outliers given the number of columns ({num_cols}) and number of unique values in each. Estimated number of combinations: {round(num_combinations):,}"
        else:
        ...
        flagged_rows_df = create_output_csv(
            ...
            explanations_1d_arr, explanations_2d_arr, explanations_3d_arr, explanations_4d_arr, explanations_5d_arr, explanations_6d_arr)
    ...
    def output_explanations(self, flagged_rows_df):
        """
        Given a dataframe with teh full information about flagged rows, return a smaller, simpler dataframe to
...
if __name__ == '__main__':
    det = CountsOutlierDetector(results_folder='Baby', results_name='toy')
